	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-center gap-4">
				...
				<div>
					<h1 className="text-2xl font-bold text-gray-900">{prd.name}</h1>
					<p className="text-sm text-gray-500">{daemonName}</p>
				</div>
			</div>
			...
			<div className="bg-white rounded-lg border border-gray-200 p-6">
				...
				<div className="mb-6">
					...
					{prd.hasBlockedStories && (
						<p className="mt-2 text-sm text-red-600">{prd.progress.blocked} blocked stories</p>
					)}
				</div>
			...
			<div className="bg-white rounded-lg border border-gray-200 p-6">
				<div className="flex items-center justify-between mb-4">
					...
					{logs.length > 0 && <span className="text-sm text-gray-500">{logs.length} entries</span>}
				</div>
				<LogViewer logs={logs} maxHeight="500px" onClear={clearLogs} />
			</div>
		</div>
	);
}
